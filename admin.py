...
@admin.register(DjangoJobCopy)
class DjangoJobCopyAdmin(admin.ModelAdmin):
    list_display = ["id", "name", "next_run_time_sec", "average_duration"]
    actions = []
    ...
    def get_queryset(self, request):
        ...
        return super(DjangoJobCopyAdmin, self).get_queryset(request)
...
class DjangoJobExecutionAdmin(admin.ModelAdmin):
    list_display = ["id", "html_status", "run_time_sec", "duration"]
    ...
    def get_queryset(self, request):
        return super(DjangoJobExecutionAdmin, self).get_queryset(request)
